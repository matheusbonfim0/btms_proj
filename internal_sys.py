...
def par(): 
    ...
    for x in range(coleta_dados.i_par): 
        print(
            coleta_dados.soup.find_all('tr', { 'class' : 'par'})[x].find_all('td')[0].text[9:-8],
            coleta_dados.soup.find_all('tr', { 'class' : 'par'})[x].find_all('td')[3].text,
            coleta_dados.soup.find_all('tr', { 'class' : 'par'})[x].find_all('td')[4].text[9:-8],
            )
